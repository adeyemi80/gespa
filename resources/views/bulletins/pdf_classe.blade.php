...
<head>
    <meta charset="UTF-8">
    <title>Bulletin - {{ $bulletins->first()['classe']->nom ?? 'Classe' }}</title>
    <style>
        @page { 
            size: A4 portrait; 
            margin: 10mm;
...
<!-- ENTOURER CHAQUE BULLETIN DANS UN DIV AVEC page-break-after -->
...
@php
    ...
    $total_eleves = $bulletin['total_eleves'] ?? 0;

    $plusFaibleMoyenne      = $bulletin['plusFaibleMoyenne'] ?? 0;
    $plusForteMoyenne       = $bulletin['plusForteMoyenne'] ?? 0;
    ...
    $afficherTiret = function($valeur) {
        return $valeur !== null && $valeur !== '' && $valeur != 0 ? number_format($valeur, 2) : '-';
    };
...
@if(isset($trimestre) && $trimestre->id == 3)
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td>
                @if($decision ?? false)
                    @if(str_contains($decision, 'Passe en'))
                        <span class="text-green-600 font-bold">{{ $decision }}</span>
                    @else
                        <span class="text-red-600 font-bold">{{ $decision }}</span>
                    @endif
                @else
                    -
                @endif
