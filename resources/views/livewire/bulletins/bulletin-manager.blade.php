{{-- resources/views/livewire/bulletins/bulletin-manager.blade.php --}}
{{-- resources/views/livewire/bulletins/bulletin-manager.blade.php --}}

<div
    @if($annee_id && $classe_id && $trimestre_id)
        wire:poll.10000ms="chargerBulletins"
    @endif
>
    {{-- ✅ Indicateur discret de polling actif --}}
    @if($annee_id && $classe_id && $trimestre_id)
        <div class="d-flex align-items-center gap-2 mb-3 text-muted" style="font-size:12px;">
            <span class="badge bg-success" style="width:8px;height:8px;border-radius:50%;padding:0;
                  animation: pulse 2s infinite;"></span>
            Actualisation automatique toutes les 10 secondes
            <span wire:loading wire:target="chargerBulletins">
    ...
    <div class="container-fluid max-w-7xl mx-auto p-6">
        ...
        <div
            wire:loading.flex
            wire:target="handleNoteOuConduiteModifiee,chargerBulletins"
            class="alert alert-info d-flex align-items-center gap-2 mb-3"
            style="display:none;"
        >
            <i class="fas fa-spinner fa-spin"></i>
            <span>Mise à jour des bulletins en cours...</span>
        </div>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-success text-white">
                    <h2 class="h5 mb-0 d-flex align-items-center gap-2">
                        ...
                        {{-- ✅ Badge "en direct" quand les bulletins sont chargés --}}
                        @if(!empty($bulletins))
                            <span class="badge bg-light text-success ms-2" style="font-size:11px;">
                                <i class="fas fa-circle me-1" style="font-size:8px;"></i> En direct
                            </span>
                        @endif
                    </h2>
                </div>
                        ...
                        <div class="col-md-3">
                            <div class="card bg-light border-0">
                                <div class="card-body text-center">
                                    ...
                                    <p class="display-6 fw-bold" style="color: #6f42c1 !important;">{{ $statistiques['meilleure_moyenne'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                                ...
                                @foreach($bulletins as $bulletin)
                                    <tr>
                                        <td class="px-4 py-3 fw-medium">
                                            {{ $bulletin['inscription']['eleve']['nom'] ?? '' }}
                                            {{ $bulletin['inscription']['eleve']['prenom'] ?? '' }}
                                        </td>
                                        <td class="px-4 py-3 text-muted small">
                                            {{ $bulletin['inscription']['eleve']['matricule'] ?? '-' }}
                                        </td>
                                        <td class="px-4 py-3 fw-bold text-primary">
                                            {{ number_format($bulletin['moyenne_trimestre'] ?? 0, 2) }}
                                        </td>
                                        <td class="px-4 py-3 small">
                                            {{ number_format($bulletin['moyenne_scientifique'] ?? 0, 2) }}
                                        </td>
                                        <td class="px-4 py-3 small">
                                            {{ number_format($bulletin['moyenne_litteraire'] ?? 0, 2) }}
                                        </td>
                                        ...
                                        <td class="px-4 py-3">
                                            <button
                                                wire:click="voirBulletin({{ $bulletin['inscription']['id'] }})"
